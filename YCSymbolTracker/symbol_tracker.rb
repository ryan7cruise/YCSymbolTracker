  ...
  def pods_target_config(pods_project, name, symbol_tracker_target)
      build_settings = Hash['FRAMEWORK_SEARCH_PATHS' => '${PODS_CONFIGURATION_BUILD_DIR}/' + name,
      'OTHER_CFLAGS' => '-fsanitize-coverage=func,trace-pc-guard',
      'OTHER_SWIFT_FLAGS' => '-sanitize=undefined -sanitize-coverage=func', 'ENABLE_BITCODE' => 'NO']
      symbol_tracker = symbol_tracker_target.product_reference
      pods_project.targets.each do |target|
          if target.name != name and !target.name.include?('Pods-') and !target.instance_of? Xcodeproj::Project::Object::PBXAggregateTarget
              # 添加依赖
